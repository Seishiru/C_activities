 ...
 ************************************************************/
 void initVHeap(VHeap *V)
 {
     ...
     for(i=0; i<VH_SIZE; i++){
		strcpy(V->VH_node[i].elem.prodID, "\0");
...
void displayVHeap(VHeap V)
{
	...
	printf("\nVHeap Address    ::  %X" , &V.VH_node );       //Complete this statemet	// MODIFIED
...
void insertSorted(cursorSet *A, product P)
{
 	if (A->count < VH_SIZE) {
        int y;
        for (y = A->elemIndex; y != -1 && strcmp(P.prodID, A->VHptr->VH_node[y].elem.prodID) != 0; y = A->VHptr->VH_node[y].next) {
        }
        if (y != -1) {
...
void deleteDict(openDic *D, char *IDen)
{
	if (D->count != 0) {
        ...
        for (x = &D->header[ndx]; *x != -1 && strcmp(D->dicVHptr->VH_node[*x].elem.prodID, IDen) != 0; x = &D->dicVHptr->VH_node[*x].next) {
        }
