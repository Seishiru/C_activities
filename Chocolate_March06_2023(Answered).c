 ...
 * Programmer  :  Cecil Raphael S. Quibranza                      *
 * Date Completed: March 02 - March 06, 2024     4:54 am          *
 ******************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void displaySet(cursorSet A) {
	...
	for(x = A.elemIndex; x != -1; x = A.VHptr->VH_node[x].next){
		printf("\n%5s%10s%13d%8d", A.VHptr->VH_node[x].elem.prodID, A.VHptr->VH_node[x].elem.prodDesc.name,A.VHptr->VH_node[x].elem.prodDesc.weight, x);
	}
 ...
 ************************************************************/
int mallocInVHeap(VHeap *VH) {
	int getSpace = VH->avail;
	if (getSpace != -1) {
		VH->avail = VH->VH_node[getSpace].next;
	}
	return getSpace;
}
...
void insertSorted(cursorSet *A, product P) {
 	if (A->count < VH_SIZE) {
		...
		if(strcmp(A->VHptr->VH_node[*x].elem.prodID, P.prodID) == 0) {
			...
			int getSpace = mallocInVHeap(A->VHptr);
			if(getSpace != -1) {
				A->count++;
				A->VHptr->VH_node[getSpace].elem = P;
				A->VHptr->VH_node[getSpace].next = *x;
				*x = getSpace;
			}
		}
	} else {
...
int openHash(char * prodID) { 
    ...
    for (i = 0; i < strlen(prodID); i++) {
        if (prodID[i] >= '0' && prodID[i] <= '9') {
            sum += prodID[i] - '0';
        }
    }

    int hashValue = sum % OPEN_DSIZE;
...
openDic convertToOpenDict(cursorSet *A) {
	openDic od = initOpenDict(A->VHptr);
	int x;
	for(x = 0; x < VH_SIZE; x++) {
		int id = openHash(A->VHptr->VH_node[x].elem.prodID);
		int *trav;
		for(trav = &od.header[id]; *trav != -1 && strcmp(A->VHptr->VH_node[x].elem.prodID, A->VHptr->VH_node[*trav].elem.prodID) > 0; trav = &od.dicVHptr->VH_node[*trav].next) {}
		// printf("\n*trav = %2d | ID: %s", *trav, A->VHptr->VH_node[x].elem.prodID);
...
void deleteDict(openDic *D, char *IDen) {
	int id = openHash(IDen);
	
	int *trav;
	for(trav = &D->header[id]; *trav != -1 && strcmp(D->dicVHptr->VH_node[*trav].elem.prodID, IDen) != 0; trav = &D->dicVHptr->VH_node[*trav].next){}
	if(*trav != -1) {
		// printf("ID %s exist\n", IDen);
		int temp = *trav;
	...
	if(CD != NULL) {
		...
		for(x = 0; x < OPEN_DSIZE; x++) {
			...
			for(trav = &D->header[x]; *trav != -1;) {
				id = closeHash(D->dicVHptr->VH_node[*trav].elem.prodID);
				temp = *trav;
				if((strcmp((*CD)[id].prodID, EMPTY)) == 0) {
					(*CD)[id] = D->dicVHptr->VH_node[*trav].elem;
				} else {
					if(L.last < LIST_SIZE) {
		...
		for(x = 0; x <= L.last; x++) {
			id = closeHash(L.prod[x].prodID);
			for(y = 0; y < CLOSE_DSIZE && strcmp((*CD)[id].prodID, EMPTY) != 0 &&  strcmp((*CD)[id].prodID, DELETED) != 0; id = (id+1)%CLOSE_DSIZE, y++) { }
			if(y < CLOSE_DSIZE) {
				(*CD)[id] = L.prod[x];
			}
		}
